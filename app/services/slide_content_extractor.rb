...
class SlideContentExtractor
  ...
  def extract
    ...
    Zip::File.open(tempfile_path) do |zip|
      ...
      slides.map.with_index(1) do |entry, idx|
        ...
        text_content = extract_text_content(slide_xml)

        # TODO: Extract notes content from slides

        # Extract images from the slide
        images = extract_images(slide_xml, rels_lookup, zip)
  ...
  def extract_text_content(slide_xml)
    # Extract all text from the slide (titles, text boxes, etc.)
    text_nodes = slide_xml.xpath("//a:t", xml_namespaces)
    text_nodes.map(&:text).join(" ").strip
  end

  def extract_images(slide_xml, rels_lookup, zip)
